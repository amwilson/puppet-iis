...
describe 'iis::manage_virtual_application', :type => :define do
  ...
  describe 'when the physical path of a virtual application differs and the path is managed' do
    let(:title) { 'mySite' }
    let(:params) {{
      :site_name                => 'myWebSite',
      :site_path                => 'C:\inetpub\wwwroot\myHost\vApp',
      :app_pool                 => 'myAppPool.example.com',
      :virtual_application_name => 'vApp',
      :manage_path              => true,
    }}

    it { should contain_exec('UpdateVirtApp-Path-myWebSite-vApp').with({
      'command' => "Import-Module WebAdministration; Set-ItemProperty \"IIS:\\Sites\\myWebSite\\vApp\" -Name physicalPath -Value \"C:\\inetpub\\wwwroot\\myHost\\vApp\"",
      'onlyif'  => "Import-Module WebAdministration; if((Test-Path \"IIS:\\Sites\\myWebSite\") -eq \$false) { exit 1 } if ((Get-ItemProperty \"IIS:\\Sites\\myWebSite\\vApp\" physicalPath) -eq \"C:\\inetpub\\wwwroot\\myHost\\vApp\") { exit 1 } else { exit 0 }",
    })}
  end

  describe 'when the physical path of a virtual application differs and the path is not managed' do
    let(:title) { 'mySite' }
    let(:params) {{
      :site_name                => 'myWebSite',
      :site_path                => 'C:\inetpub\wwwroot\myHost\vApp',
    ...
    it { should_not contain_exec('UpdateVirtApp-Path-myWebSite-vApp')}
  end
end
